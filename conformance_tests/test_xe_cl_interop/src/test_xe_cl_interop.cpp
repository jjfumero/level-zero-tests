...
protected:
  ...
  ~xeCLInteropTests() { clReleaseMemObject(cl_memory_); }
...
static std::vector<int> bitonic_sort_cl(std::vector<int> input) {
  ...
  const std::vector<uint8_t> binary_file =
      lzt::load_binary_file("xe_bitonic_sort.spv");
  ...
  cl_kernel kernel = clCreateKernel(program, "xe_bitonic_sort", &ret);
...
static std::vector<int> bitonic_sort_l0(std::vector<int> input) {
  ...
  auto func = lzt::create_function(l0_mod, "xe_bitonic_sort");
